...
class pyvk:
    def __init__(self, email, password, key, permissions):
        #("mail@mail.com", "password", "messages,photos")
        self.email = email
        self.password = password
        self.token, self.user_id = vk_auth.auth(email, password, key, permissions)
    ...
    def get(self, users_id, fields=[]):
        if type(users_id) == int:
            users_id = [users_id]
        result = self.call_api("users.get", {"uids": ",".join(map(str, users_id)), "fields": ",".join(fields)})
        return result
    
    def send(self, typeid, id, message):
        if message != "":
            return self.call_api("messages.send", {typeid: id, "message": message})
        else:
            return
        
    def setActivity(self, typeid, id):
        return self.call_api("messages.setActivity", {typeid: id, "type": "typing"})
    
    def getLastActivity(self, user_id):
        return self.call_api("messages.getLastActivity", ("uid", user_id))
